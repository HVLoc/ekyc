...
class NfcDialogController extends BaseLoadingGetx {
  ...
  Future<void> scanNFC() async {
    ...
    if (sendNfcRequestModel.number != null) {
      ...
      appController.sendNfcRequestModel = sendNfcRequestModel;
      if (sendNfcRequestModel.residentVMN != "") {
        if (appController.deepLinkModel.typeSignPdf == AppConst.signeKycNfc ||
            appController.deepLinkModel.typeSignPdf ==
                AppConst.signNFCEKYCCert) {
          Get.toNamed(
      ...
      if (errorString == AppConst.nfcFailDocumentNumber) {
        showFlushNoti(
            errorString == AppConst.nfcFailDocumentNumber
                ? LocaleKeys.nfcDialog_introduceScanNfcError.tr
                : "${LocaleKeys.nfcDialog_nfcFailTitle1.tr}$idDocument${LocaleKeys.nfcDialog_nfcFailTitle2.tr}",
            isSuccess: false,
            duration: errorString == AppConst.nfcFailDocumentNumber
  ...
  void setupData() {
    ...
    try {
      ...
      //   }
      // } else {
      idDocument = (appController.deepLinkModel.documentNumberSign == "")
          ? appController.user.identityNumber
          : appController.deepLinkModel.documentNumberSign;
      // if (appController.documentNumberSign.length == 13) {
      //   idDocument = appController.documentNumberSign;
      // }else{
      //   // idDocument = "038099006031";
      // }
  ...
  Future<void> readMRTD({bool isPace = true}) async {
    try {
      ...
      await nfc.setIosAlertMessage(
          formatProgressMsg(LocaleKeys.nfcDialog_introduceScanNfc2.tr, 0));
      // try {
      //   mrtdDataTemp.cardAccess = await passport.readEfCardAccess();
      // } on PassportError {
      //   //if (e.code != StatusWord.fileNotFound) rethrow;
      // }
      // nfc.setIosAlertMessage(LocaleKeys.nfcDialog_nfc_introduceScanNfc3.tr);
      ...
      //   // if (e.code != StatusWord.fileNotFound) rethrow;
      // }
      // await nfc.setIosAlertMessage(LocaleKeys.nfcDialog_nfc_introduceScanNfc4.tr, 20);
      if (isPace) {
